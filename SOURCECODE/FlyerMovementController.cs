...
- FlyerMovementController [modified class]

1) Incorporated oculus touch controller input.
2) Removed head rotation control scheme.
3) Added joystick control scheme with automatic recenter.
...
namespace VRStandardAssets.Flyer
{
    ...
    {
        [SerializeField] private float m_xDisplacement = 30f;       // EDIT
        [SerializeField] private float m_yDisplacement = 20f;       // EDIT
        [SerializeField] private float m_DistanceFromCamera = 75f;  // The distance from the camera the ship aims to be.
        [SerializeField] public float m_Speed = 100f;              // The speed the ship moves forward.
        ...
        private IEnumerator MoveFlyer ()
        {
            while (m_IsGameRunning)
            {
                ...
                //Control scheme now uses the left thumbstick, [Float modifier range -1,1]. Automatic centering of dolphin.
                Quaternion headRotation = Quaternion.Euler(-Input.GetAxis("LS Vertical")* m_yDisplacement, Input.GetAxis("LS Horizontal")* m_xDisplacement, 0 );
                m_TargetMarker.position = m_Camera.position + (headRotation * Vector3.forward) * m_DistanceFromCamera;
